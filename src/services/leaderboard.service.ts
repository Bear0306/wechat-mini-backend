...
export async function getLeaderboardPage(
  ...
  const userIds = slice.map((g: { userId: number }) => g.userId);
  const users = await prisma.user.findMany({
    where: { id: { in: userIds } },
    select: { id: true, wechatNick: true, avatarUrl: true },
  });
  const uMap = new Map(users.map((u: { id: number, wechatNick: string | null, avatarUrl: string | null }) => [u.id, u]));

  const list: RankRow[] = slice.map((e: { userId: number, steps: number, user: { id: number, wechatNick: string | null, avatarUrl: string | null } }, i: number) => {
...
export async function getMyRank(
  ...
  const idx = entries.findIndex((e: { userId: number }) => e.userId === userId);
...
export async function finalize(contestId: number) {
  ...
  console.log(`Finalizing leaderboard for contest ${contestId}: top ${topJson}, tail ${backupJson}, hash ${integrityHash}`);
