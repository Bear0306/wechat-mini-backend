...
//     const m = await upsertMembership(req.user!.id, body.tier as MembershipTier, body.months);
...
router.get('/me', async (req: any, res) => {
  ...
  const joinCount = credits.reduce((acc: number, c: { qty: number, consumedQty: number }) => acc + Math.max(0, c.qty - c.consumedQty), 0);
