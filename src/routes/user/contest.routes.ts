...
import { getUserContestList } from '../../services/contest.service';
...
router.get('/recent-list', async (req, res) => {
  try {
    ...
    const items3 = await getUserContestList(uid, 3);
...
router.get('/list', async (req, res) => {
  const uid = Number(req.user?.id?? 5);
  const allItems = await getUserContestList(uid, 0);
...
router.get('/ended', async (req, res) => {
  ...
  const participatedIds = (await prisma.contestEntry.findMany({
    ...
    distinct: ['contestId']
  })).map((x: { contestId: number }) => x.contestId);
  ...
  // 装饰
  type Contest = { id: number, title: string, startAt: Date, endAt: Date, rewardTopN: number };
  const itemsAll = await Promise.all(ended.map(async (c: Contest) => {
    ...
    if (my) {
      ...
      myRank = better + 1;
      canClaim = myRank != null && myRank <= c.rewardTopN;
